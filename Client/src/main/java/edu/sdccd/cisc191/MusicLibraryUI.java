...
public class MusicLibraryUI extends VBox {
    ...
    /*
     **If there is a current track playing, setCurrentTrack stops the playback,
     * and removes the timeListener from the track's media player.
     **The new track is set as the current track.
     **If the new current track is not null, the following actions are performed:
     * a. UI controls (slider and label) are set for the new track.
     * b. The slider is bound to the new track's media player.
     * c. A timeListener is set up to update the slider and duration label as the track plays.
     * d. The timeListener is added to the new track's media player.
     * e. The track's title is displayed in the titleField.
     * In case of any errors during the process, such as stopping the current track or setting UI controls,
     * the method will output an error message to the console.
     *
